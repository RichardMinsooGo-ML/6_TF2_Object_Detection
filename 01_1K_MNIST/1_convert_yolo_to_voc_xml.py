...
## converts coco into xml 
def xml_transform_test(classes):  
    class_path  = join('txt_test')
    ids = list()
    l=os.listdir(class_path)
    ...
    annopath = join('txt_test', '%s.txt')
    imgpath = join('mnist_test', '%s.jpg')
    
    os.makedirs(join('xml_test'), exist_ok=True)
    outpath = join('xml_test', '%s.xml')
...
xml_transform_test(YOLO_CLASSES)
