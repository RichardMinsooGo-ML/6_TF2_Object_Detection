...
from yolo_core.yolov3 import Create_Yolov3, compute_loss
...
def main():
    ...
    model = Create_Yolov3(input_size=YOLO_INPUT_SIZE, training=True, CLASSES=TRAIN_CLASSES)
    ...
    mAP_model = Create_Yolov3(input_size=YOLO_INPUT_SIZE, CLASSES=TRAIN_CLASSES) # create second model to measure mAP
        ...
        for image_data, target in trainset:
            ...
            if (cur_step+1)%1000 == 0 or (cur_step+1) == steps_per_epoch:
                ...
                if TRAIN_SAVE_CHECKPOINT and not TRAIN_SAVE_BEST_ONLY:
                    save_directory = os.path.join(TRAIN_CHECKPOINTS_FOLDER, DATA_TYPE+"_val_loss_{:7.2f}".format(total_val/count))
                    model.save_weights(save_directory)
